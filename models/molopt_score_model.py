...
def cosine_beta_schedule(timesteps, s=0.008):
    ...
    alphas = torch.clip(alphas, 0.001, 1.0)
...
def log_normal(values, means, log_scales):
    ...
    log_prob = (
        ...
        - log_scales
        - torch.log(torch.sqrt(2 * torch.pi))
    )
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        self.sqrt_alphas_cumprod = to_torch_const(torch.sqrt(alphas_cumprod))
        ...
        self.posterior_logvar = to_torch_const(
            torch.log(
                torch.cat(
                    (torch.tensor([self.posterior_var[1]]), self.posterior_var[1:])
                )
            )
        )
        ...
        if config.v_beta_schedule == "cosine":
            ...
            raise NotImplementedError
        log_alphas_v = torch.log(alphas_v)
        log_alphas_cumprod_v = torch.cumsum(log_alphas_v, dim=0)
    ...
    def q_v_pred_one_timestep(self, log_vt_1, t, batch):
        ...
        log_probs = log_add_exp(
            ...
            log_1_min_alpha_t - torch.log(torch.tensor(self.num_classes)),
        )
        return log_probs
    ...
    def q_v_pred(self, log_v0, t, batch):
        ...
        log_probs = log_add_exp(
            ...
            log_1_min_cumprod_alpha - torch.log(torch.tensor(self.num_classes)),
        )
        return log_probs
    ...
    def get_diffusion_loss(
        ...
        if time_step is None:
            ...
            pt = torch.ones_like(time_step).float() / self.num_timesteps
        time_step = time_step.to(self.alphas_cumprod.device)
        idx = torch.tensor(0, device=time_step.device)
        a = self.alphas_cumprod.index_select(idx, time_step)  # (num_graphs, )
